//==============================================================================
// CellML file:   C:\Program Files\COR\Models\benson_epicardial_2008.cellml
// CellML model:  benson_epicardial_2008
// Date and time: 23/07/2015 at 15:34:33
//------------------------------------------------------------------------------
...
Unit benson_epicardial_2008_unit;
...
Type
   Tbenson_epicardial_2008_unit = Class
      Public
         //---------------------------------------------------------------------
         // State variables
         //---------------------------------------------------------------------
         ...
         tissue: Double;   // dimensionless (in Environment)
...
Procedure Tbenson_epicardial_2008_unit.Init;
Begin
   //---------------------------------------------------------------------------
   // State variables
   //---------------------------------------------------------------------------

   Y[0] := 0.00012131666;   // Ca_i (mM) (in Ca)
   Y[1] := 1.737580994071;   // Ca_jsr (mM) (in Ca)
   Y[2] := 1.832822335168;   // Ca_nsr (mM) (in Ca)
   Y[3] := 0.00012271265;   // Ca_ss (mM) (in Ca)
   Y[4] := 15.59207157178;   // Cl_i (mM) (in Cl)
   Y[5] := 0.00000164013;   // d (dimensionless) (in ICaL)
   Y[6] := 8.98230672628;   // dp (dimensionless) (in ICaL)
   Y[7] := 0.999961508634;   // f1 (dimensionless) (f in ICaL)
   Y[8] := 0.992234519148;   // f2 (dimensionless) (in ICaL)
   Y[9] := 0.97836624923;   // fca (dimensionless) (in ICaL)
   Y[10] := 0.893052931249;   // fca2 (dimensionless) (in ICaL)
   Y[11] := 0.00000724074;   // xr (dimensionless) (in IKr)
   Y[12] := 0.019883138161;   // xs1 (dimensionless) (in IKs)
   Y[13] := 0.019890650554;   // xs2 (dimensionless) (in IKs)
   Y[14] := 0.26130711759;   // hL (dimensionless) (in INal)
   Y[15] := 0.001356538159;   // mL (dimensionless) (in INal)
   Y[16] := 0.987317750543;   // H (dimensionless) (in INa)
   Y[17] := 0.991924983076;   // J (dimensionless) (in INa)
   Y[18] := 0.001356538159;   // m (dimensionless) (in INa)
   Y[19] := 0.021123704774;   // CaMKtrap (dimensionless) (in Irel)
   Y[20] := 0.862666650318;   // ri (dimensionless) (in Irel)
   Y[21] := 0.0;   // ro (dimensionless) (in Irel)
   Y[22] := 0.000816605172;   // AA (dimensionless) (in Ito2)
   Y[23] := 0.013970786703;   // ydv (dimensionless) (in Ito)
   Y[24] := 0.99996472752;   // ydv2 (dimensionless) (in Ito)
   Y[25] := 0.829206149767;   // zdv (dimensionless) (in Ito)
   Y[26] := 135.469546216758;   // K_i (mM) (in K)
   Y[27] := 12.972433387269;   // Na_i (mM) (in Na)
   Y[28] := -85.781844107117;   // V (mV) (in cell)
   ...
   cbar := 50.0e-3;   // mM (in Ca)
   csqnbar := 10.0;   // mM (in Ca)
   kmc := 2.38e-3;   // mM (in Ca)
   kmcsqn := 0.8;   // mM (in Ca)
   kmt := 0.5e-3;   // mM (in Ca)
   tbar := 70.0e-3;   // mM (in Ca)
   Ca_o := 1.8;   // mM (in Environment)
   Cl_o := 100.0;   // mM (in Environment)
   F2 := 96485.0;   // C_per_mole (F in Environment)
   FonRT := 0.0374358835078;   // per_mV (in Environment)
   K_o := 5.4;   // mM (in Environment)
   Na_o := 140.0;   // mM (in Environment)
   tissue := 0.0;   // dimensionless (in Environment)
   gacai := 1.0;   // dimensionless (in ICaL)
   gacao := 0.341;   // dimensionless (in ICaL)
   pca := 2.43e-4;   // L_per_F_ms (in ICaL)
   GClb := 2.25e-4;   // mS_per_uF (in IClb)
   KmCa := 1.25e-4;   // mM (in INaCa)
   KmCai := 0.0036;   // mM (in INaCa)
   KmCao := 1.3;   // mM (in INaCa)
   KmNai1 := 12.3;   // mM (KmNai in INaCa)
   KmNao := 87.5;   // mM (in INaCa)
   NCXmax := 4.5;   // uA_per_uF (in INaCa)
   ...
   GNaL := 65.0e-4;   // mS_per_uF (in INal)
   ibarpca := 0.0575;   // uA_per_uF (in IpCa)
   kmpca := 0.5e-3;   // mM (in IpCa)
   ...
   a := 0.0011;   // cm (in cell)
   l := 0.01;   // cm (in cell)
   stim_amplitude := -20.0;   // uA_per_uF (in cell)
   stim_duration := 3.0;   // ms (in cell)
   stim_offset := 0.0;   // ms (in cell)
   stim_period := 1.0e3;   // ms (in cell)
...
Procedure Tbenson_epicardial_2008_unit.Compute(Const time: Double);
Begin
   // time: time (ms)

   bmyo := 1.0/(1.0+cbar*kmc/Sqr(Y[0]+kmc)+kmt*tbar/Sqr(Y[0]+kmt));
   ICab := 1.995084e-7*4.0*Y[28]*F2*FonRT*(Y[0]*Exp(2.0*Y[28]*FonRT)-0.341*Ca_o)/(Exp(2.0*Y[28]*FonRT)-1.0);
   ...
   Else
      ICaL := Y[5]*Y[7]*Y[8]*Y[9]*Y[10]*ibarca;
   ...
   IK1 := 0.5*Sqrt(K_o/5.4)*ak1/(ak1+bk1)*(Y[28]-EK);
   IKp := 0.00276*(Y[28]-EK)/(1.0+Exp((7.488-Y[28])/5.98));
   r := 1.0/(1.0+Exp((Y[28]+10.0)/15.4));
   IKr := gkr*Y[11]*r*(Y[28]-EK);
   xrss := 1.0/(1.0+Exp(-(Y[28]+10.085)/4.25));
   tauxr := 1.0/(6.0e-4*(Y[28]-1.7384)/(1.0-Exp(-0.136*(Y[28]-1.7384)))+3.0e-4*(Y[28]+38.3608)/(Exp(0.1522*(Y[28]+38.3608))-1.0));
   dY[11] := (xrss-Y[11])/tauxr;
   gks := 0.0248975*(1.0+0.6/(1.0+Power(3.8e-5/Y[0], 1.4)));
   ...
   ay3 := 0.0039/(1.0+Exp((Y[28]+63.0)/5.0));
